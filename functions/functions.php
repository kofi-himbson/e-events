...
$check = checkForTickets($event_id);
if($check != false){
//if the number of rows is greater than 0
	if($result->num_rows>0){
	//for each row, echo the following html code
		...
		<div class="col-lg-6 product-details pl-md-5 ftco-animate">
			...
			<div class="rating d-flex">
				<p class="text-left mr-4">
					<a href="#" class="mr-2" style="color: #000;">'.$row['tickets_available'].' <span style="color: #bbb;">Tickets Issued</span></a>
				</p>
				<p class="text-left">
	...
	<div class="col-lg-6 mb-5 ftco-animate">
		...
		<div class="col-lg-6 product-details pl-md-5 ftco-animate">
			...
			<p class="price"><span style="color: #FF0000;text-decoration: line-through;" >GH¢'.$row['event_price'].'.00</span></p>
			<h3 style="color: #FF0000;">SOLD OUT</h3>
			<p>'.$row['event_desc'].'
				</p>
...
function subtotal(){
	$conn = openDatabase();
	$sql = "select cart_id,event_price*quantity AS total from event,cart where event.event_id=cart.event_id";
				...
				<div class="col-md-6 col-lg-3 ftco-animate">
    				<div class="product">
    					...
    					<div class="text py-3 pb-4 px-3 text-center">
    						...
    						<div class="d-flex">
    							<div class="pricing">
		    						<p class="price"><span class="price-sale">Book Here</span></p>
		    					</div>
	    					</div>
...
function displayoneConference($conf_id){
	...
	<div class="col-lg-6 mb-5 ftco-animate">
			...
			<div class="col-lg-6 product-details pl-md-5 ftco-animate">
				...
				<div class="rating d-flex">
					<p class="text-left mr-4">
						<a href="#" class="mr-2" style="color: #000;">'.$row['conf_seats_available'].' <span style="color: #bbb;"> Seats at Venue</span></a>
					</p>
					<p class="text-left">
...
function checkForTickets($id){
//open the connection
$conn = openDatabase();
//sql query to get all the products from the database
$result = $conn->query("SELECT tickets_left FROM event where event_id=$id");
//if the number of rows is greater than 0
if($result->num_rows>0){
	//for each row, echo the following html code
	while ($row=$result->fetch_assoc()) {
		if( $row['tickets_left'] <= 0){
			return false;
		}else{
		
			return $row['tickets_left'];
		}
...
function eventName(){
	$conn = openDatabase();
	$sql = "select event_name from event,cart where event.event_id=cart.event_id";
	$result = $conn->query($sql);
	$row = $result->fetch_assoc();
	if ($result->num_rows > 0) {
